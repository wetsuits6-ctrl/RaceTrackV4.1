...
if (missingEnvs.length > 0) {
    console.error(`⚠️ Missing required environment variables: ${missingEnvs.join(', ')}`);
...
try {
  if (fs.existsSync(LAST_RACE_FILE)) {
    ...
    console.log('✅ Loaded last race data from file');
  }
} catch (err) {
  console.error('❌ Error loading last race data:', err);
}
...
function saveLastRace(raceData) {
  try {
    ...
    console.log('💾 Last race data saved to file');
  } catch (err) {
    console.error('❌ Error saving last race data:', err);
  }
}
...
function requireAuth(role) {
    return (req, res, next) => {
        ...
        if (req.session.destroyed) {
            console.log('⚠️ Session has been destroyed');
            return res.redirect('/');
        }
        ...
        if (isAuthenticated) {
            ...
            console.log(`⚠️ Unauthorized ${role} access attempt from IP: ${req.ip}`);
...
app.post('/login', (req, res) => {
    ...
    if (password === process.env.RECEPTIONIST_KEY) {
        ...
        console.log('⚠️ Invalid login attempt');
